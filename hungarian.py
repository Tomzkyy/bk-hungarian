...
with tab1:
    ...
    age = st.number_input(
        label="**Age**", min_value=28, max_value=66)
    st.write(
        f"Min value: **28** | Max value:   **66**")
    st.write("")
    ...
    cp_sb = st.selectbox(label="**Chest Pain Type**", options=[
    ...
    trestbps = st.number_input(label="**Resting Blood Pressure**",
                               min_value=df_final['trestbps'].min(), max_value=df_final['trestbps'].max())
    st.write(
        f"Min value: **{df_final['trestbps'].min()}** | Max value: **{df_final['trestbps'].max()}**")
    st.write("")

    chol = st.number_input(label="**Serum Cholestoral** (in mg/dl)]",
                           min_value=df_final['chol'].min(), max_value=df_final['chol'].max())
    st.write(
        f"Min value: **{df_final['chol'].min()}** | Max value: **{df_final['chol'].max()}**")
    st.write("")
    ...
    restecg_sb = st.selectbox(label="**Resting Electrocardiographic Results**", options=[
    ...
    thalach = st.number_input(
        label="**Maximum Heart Rate Achieved**", min_value=df_final['thalach'].min(), max_value=df_final['thalach'].max())
    st.write(
        f"Min value: **{df_final['thalach'].min()}** | Max value: **{df_final['thalach'].max()}**")
    st.write("")
    ...
    oldpeak = st.number_input(label="**ST Depression Induced By Exercise Relative To Rest**",
                              min_value=df_final['oldpeak'].min(), max_value=df_final['oldpeak'].max())
    st.write(
        f"Min value: **{df_final['oldpeak'].min()}** | Max value: **{df_final['oldpeak'].max()}**")
    st.write("")
